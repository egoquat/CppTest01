...
class SkynetStrikesBack
{
	...
	public:
		...
		static void RemoveLink(const int nodeA, const int nodeB)
		{
			...
			for (uint i = 0; i < _goals.size(); ++i)
			{
				...
				GetNode(_goals[i])->MappingWeightR(0);
			}
		}
	...
	public:
			...
			void AdditionToWeight()
			{
				...
				{
					isExist = IsExist(_goals, _adjacents[i]);
					if (true == isExist)
					{
						break;
					}
			...
			uint GetWeightToGoal()
			{
				return _weightToGoal;
			}
	...
	private:
		...
		void MappingWeightR(const uint depth)
		{
			uint D = depth + 1;
			if ( 2 >= D)
			{
...
int SkynetStrikesBack::Tree::_count = 0;
SkynetStrikesBack::Tree* SkynetStrikesBack::Tree::_root = nullptr;
SkynetStrikesBack::Tree** SkynetStrikesBack::Tree::_nodes = nullptr;
vector<int> SkynetStrikesBack::Tree::_goals;
